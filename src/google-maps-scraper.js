...
//   // This matches a 2-letter state abbreviation before the ZIP code
...
//     console.log(`\n=== PHASE 1: COLLECTING PLACE URLs for search query: "${query}" ===`);
...
//     console.log(`✅ Collected so far: ${resultUrls.length}`);
//   }
...
//     console.log(`\n📦 Processing chunk ${chunkIndex + 1} of ${chunkedUrls.length} (${chunk.length} URLs)...`);
...
//       console.log(`🧭 Scraping tab group ${groupIndex + 1} of ${tabGroups.length}...`);
...
//               return h1 ? h1.textContent.replace(/^[^\w\d+]+/, '').replace(/\s+/g, ' ').trim() : null;
//             };

//             const getRating = () => {
//               const ratingElement = document.evaluate(
//                 "//div[@class='F7nice ']/span/span",
//                 document,
//                 null,
...
//                 "(//div[@class='F7nice ']/span)[2]/span/span",
//                 document,
//                 null,
//                 XPathResult.FIRST_ORDERED_NODE_TYPE,
//                 null
//               ).singleNodeValue;
//               return reviewElement ? reviewElement.textContent.trim().replace(/[()]/g, '') : null;
//             };
...
//             if (abbreviation) {
//               state = stateAbbrToFull[abbreviation.toUpperCase()] || null;
//             }
//           }
...
//             console.log(`📊 Scraped ${results.length} places so far...`);
//           }
//         } catch (err) {
//           //progressBar.stop();
//           console.warn(`❌ Error scraping ${url}:\n  ${err.message}`);
//           //progressBar.start(resultUrls.length, progressBar.value + 1);
//         } finally {
